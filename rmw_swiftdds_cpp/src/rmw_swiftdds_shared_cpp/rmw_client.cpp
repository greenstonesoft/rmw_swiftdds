...
rmw_ret_t __rmw_destroy_client(const char * identifier, rmw_node_t * node, rmw_client_t * client)
{
  ...
  // Update graph
  greenstone::dds::GUID writer_guid;
  writer_guid.from_octet16(info->request_writer_->get_instance_handle().value);
  rmw_gid_t request_publisher_gid =
    rmw_swiftdds_shared_cpp::create_rmw_gid(identifier, writer_guid);
  greenstone::dds::GUID reader_guid;
  reader_guid.from_octet16(info->response_reader_->get_instance_handle().value);
  rmw_gid_t response_subscriber_gid =
    rmw_swiftdds_shared_cpp::create_rmw_gid(identifier, reader_guid);

  {
    // Update graph
    std::lock_guard<std::mutex> guard(common_context->node_update_mutex);
    rmw_gid_t gid = rmw_swiftdds_shared_cpp::create_rmw_gid(
      identifier, writer_guid);
    common_context->graph_cache.dissociate_writer(
      gid,
      common_context->gid,
      node->name,
      node->namespace_);
    gid = rmw_swiftdds_shared_cpp::create_rmw_gid(
      identifier, reader_guid);
    rmw_dds_common::msg::ParticipantEntitiesInfo msg =
      common_context->graph_cache.dissociate_reader(
      gid, common_context->gid, node->name, node->namespace_);
    final_ret = rmw_swiftdds_shared_cpp::__rmw_publish(
      identifier,
      common_context->pub,
  ...
  // Delete DataWriter and DataReader
  {
    ...
    greenstone::dds::ReturnCode_t ret =
      participant_info->subscriber_->delete_datareader(info->response_reader_);
    if (ret != greenstone::dds::ReturnCode_t::RETCODE_OK) {
      ...
      info->response_reader_->set_listener(nullptr, greenstone::dds::EmptyStatusMask);
    }
    ...
    if (ret != greenstone::dds::ReturnCode_t::RETCODE_OK) {
      ...
      final_ret = RMW_RET_ERROR;
      info->request_writer_->set_listener(nullptr, greenstone::dds::EmptyStatusMask);
    }
  ...
  rmw_qos_profile_t * qos)
{
  ...
  greenstone::dds::DataWriter * swiftdds_rw = cli->request_writer_;
  greenstone::dds::DataWriterQos wqos;
  swiftdds_rw->get_qos(wqos);
  qos->lifespan = dds_duration_to_rmw(wqos.life_span().duration());
  dds_qos_to_rmw_qos(wqos, qos);
  return RMW_RET_OK;
}
...
{
  auto cli = static_cast<CustomClientInfo *>(client->data);
  greenstone::dds::DataReader * swiftdds_dr = cli->response_reader_;
  greenstone::dds::DataReaderQos rqos;
  swiftdds_dr->get_qos(rqos);
  dds_qos_to_rmw_qos(rqos, qos);
  return RMW_RET_OK;
}
