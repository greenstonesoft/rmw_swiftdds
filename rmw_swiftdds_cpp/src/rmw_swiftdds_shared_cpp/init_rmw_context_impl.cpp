...
rmw_ret_t rmw_swiftdds_shared_cpp::decrement_context_impl_ref_count(rmw_context_t * context)
{
  ...
  ret = rmw_swiftdds_shared_cpp::join_listener_thread(context);
  if (RMW_RET_OK != ret) {
    return ret;
  }
  ...
  if (!common_context->graph_cache.remove_participant(common_context->gid)) {
    RMW_SAFE_FWRITE_TO_STDERR(
      RCUTILS_STRINGIFY(__function__) ":" RCUTILS_STRINGIFY(
        __line__) ": "
      "couldn't remove Participant gid from graph_cache when destroying Participant");
  }

  ret = rmw_swiftdds_shared_cpp::destroy_subscription(
  ...
  err = rmw_swiftdds_shared_cpp::destroy_publisher(
  ...
  err = rmw_swiftdds_shared_cpp::destroy_participant(participant_info);
  if (RMW_RET_OK != ret && RMW_RET_OK != err) {
    RMW_SAFE_FWRITE_TO_STDERR(
      RCUTILS_STRINGIFY(__function__) ":" RCUTILS_STRINGIFY(
  ...
  if (RMW_RET_OK != rmw_swiftdds_shared_cpp::__rmw_destroy_guard_condition(
