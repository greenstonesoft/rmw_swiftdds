...
rmw_ret_t rmw_swiftdds_shared_cpp::run_listener_thread(rmw_context_t * context)
{
  ...
  common_context->listener_thread_gc =
    rmw_swiftdds_shared_cpp::__rmw_create_guard_condition(context->implementation_identifier);
  ...
  if (common_context->listener_thread_gc) {
    if (RMW_RET_OK !=
      rmw_swiftdds_shared_cpp::__rmw_destroy_guard_condition(common_context->listener_thread_gc))
    {
      RCUTILS_SAFE_FWRITE_TO_STDERR(
        RCUTILS_STRINGIFY(__FILE__) ":" RCUTILS_STRINGIFY(
...
rmw_ret_t rmw_swiftdds_shared_cpp::join_listener_thread(rmw_context_t * context)
{
  auto common_context = static_cast<rmw_dds_common::Context *>(context->impl->common);
  common_context->thread_is_running.exchange(false);
  rmw_ret_t rmw_ret = rmw_swiftdds_shared_cpp::__rmw_trigger_guard_condition(
  ...
  { \
    RCUTILS_SAFE_FWRITE_TO_STDERR( \
      RCUTILS_STRINGIFY(__FILE__) ":" RCUTILS_STRINGIFY(__function__) ":" RCUTILS_STRINGIFY( \
        __LINE__) RCUTILS_STRINGIFY(msg) ": ros discovery info listener thread will shutdown " \
      "...\n"); \
  }
...
void node_listener(rmw_context_t * context)
{
  ...
  rmw_wait_set_t * wait_set = rmw_swiftdds_shared_cpp::__rmw_create_wait_set(
  ...
  while (common_context->thread_is_running.load()) {
    ...
    if (RMW_RET_OK != rmw_swiftdds_shared_cpp::__rmw_wait(
    ...
    if (subscriptions_buffer[0]) {
      ...
      while (taken) {
        if (RMW_RET_OK != rmw_swiftdds_shared_cpp::__rmw_take(
            context->implementation_identifier,
            common_context->sub,
  ...
  if (RMW_RET_OK != rmw_swiftdds_shared_cpp::__rmw_destroy_wait_set(
