  ...
  rmw_publisher_t * publisher)
{
...

  {
    ...
    rmw_ret_t publish_ret = rmw_swiftdds_shared_cpp::__rmw_publish(
      identifier,
      common_context->pub,
      &msg,
      nullptr);
  ...
  rmw_time_t wait_timeout)
{
  ...
  greenstone::dds::Duration_t timeout = rmw_time_to_swiftdds(wait_timeout);

  greenstone::dds::ReturnCode_t ret = info->data_writer_->wait_for_acknowledgements(timeout);
...
{
  auto info = static_cast<CustomPublisherInfo *>(publisher->data);
  greenstone::dds::DataWriter * swiftdds_dw = info->data_writer_;
  greenstone::dds::DataWriterQos dds_qos;
  swiftdds_dw->get_qos(dds_qos);
  ...
  void ** ros_message)
{
  ...
  if (info->type_support_ != nullptr) {
    if (info->type_support_->is_plain_types() == false) {
      return RMW_RET_ERROR;
    }
  }
  uint32_t const msg_size{info->type_support_->data_size_of()};
  if (info->data_writer_->loan_sample(*ros_message, msg_size) !=
    greenstone::dds::ReturnCode_t::RETCODE_OK)
  {
    return RMW_RET_ERROR;
  }
  ...
  void * loaned_message)
{
  ...
  auto info = static_cast<CustomPublisherInfo *>(publisher->data);
  if (info->data_writer_->return_loan(loaned_message) !=
    greenstone::dds::ReturnCode_t::RETCODE_OK)
  {
    return RMW_RET_ERROR;
  }
