...
void CustomDataReaderListener::on_subscription_matched(
  ...
  const greenstone::dds::SubscriptionMatchedStatus & info) noexcept
{
  (void)reader;
  ...
  if(info.current_count_change() == 1) {
    subscription_event_->track_unique_publisher(endpoint_guid);
  } else if(info.current_count_change() == -1) {
    subscription_event_->untrack_unique_publisher(endpoint_guid);
  } else {
    return;
  }

  subscription_event_->update_matched(info.total_count(),
                                      info.total_count_change(),
                                      info.current_count(),
                                      info.current_count_change());
}

void CustomDataReaderListener::on_data_available(greenstone::dds::DataReader *reader) noexcept
{
  (void)reader;
...
void CustomDataReaderListener::on_requested_deadline_missed(
  greenstone::dds::DataReader *reader,
  const greenstone::dds::RequestedDeadlineMissedStatus & status) noexcept
{
  (void)reader;

  subscription_event_->update_requested_deadline_missed(status.total_count(),
                                                        status.total_count_change());
}

void CustomDataReaderListener::on_liveliness_changed(
  greenstone::dds::DataReader *reader,
  const greenstone::dds::LivelinessChangedStatus & status) noexcept
{
  (void)reader;

  subscription_event_->update_liveliness_changed(status.alive_count(),
...
void CustomDataReaderListener::on_sample_lost(
  greenstone::dds::DataReader *reader, const greenstone::dds::SampleLostStatus & status) noexcept
{
  (void)reader;
...
void CustomDataReaderListener::on_requested_incompatible_qos(
  greenstone::dds::DataReader *reader,
  const greenstone::dds::RequestedIncompatibleQosStatus & status) noexcept
{
  (void)reader;
...
greenstone::dds::StatusCondition * RMWSubscriptionEvent::get_statuscondition() const
{
...
{
  assert(rmw_swiftdds_shared_cpp::internal::is_event_supported(event_type));
  ...
  switch(event_type) {
    ...
    case RMW_EVENT_REQUESTED_QOS_INCOMPATIBLE: {
        ...
        rmw_data->last_policy_kind =
          rmw_swiftdds_shared_cpp::internal::dds_qos_policy_to_rmw_qos_policy(
    ...
    case RMW_EVENT_SUBSCRIPTION_MATCHED: {
        ...
        greenstone::dds::SubscriptionMatchedStatus matched_status;
...
void RMWSubscriptionEvent::track_unique_publisher(greenstone::dds::GUID guid)
{
  std::lock_guard<std::mutex> lock(publishers_mutex_);
  publishers_.insert(guid);
}

void RMWSubscriptionEvent::untrack_unique_publisher(greenstone::dds::GUID guid)
{
  std::lock_guard<std::mutex> lock(publishers_mutex_);
  ...
  uint32_t not_alive_count_change)
{
  ...
  // Accumulate deltas
  liveliness_changed_status_.alive_count_change(liveliness_changed_status_.alive_count_change() +
                                                alive_count_change);
  liveliness_changed_status_.not_alive_count_change(
...
void RMWSubscriptionEvent::update_sample_lost(uint32_t total_count, uint32_t total_count_change)
{
  ...
  // Accumulate deltas
  sample_lost_status_.total_count_change(sample_lost_status_.total_count_change() +
                                         total_count_change);
  ...
  uint32_t total_count_change)
{
  ...
  // Accumulate deltas
  incompatible_qos_status_.total_count_change(incompatible_qos_status_.total_count_change() +
                                              total_count_change);
  ...
  uint32_t total_count_change)
{
  ...
  // Accumulate deltas
  inconsistent_topic_status_.total_count_change(inconsistent_topic_status_.total_count_change() +
                                                total_count_change);
  ...
  int32_t current_count_change)
{
  ...
  matched_status_.total_count_change(matched_status_.total_count_change() + total_count_change);
  matched_status_.current_count(current_count);
  matched_status_.current_count_change(matched_status_.current_count_change() +
