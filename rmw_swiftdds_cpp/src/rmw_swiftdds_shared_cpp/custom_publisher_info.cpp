...
void CustomDataWriterListener::on_publication_matched(
  ...
  const greenstone::dds::PublicationMatchedStatus & status) noexcept
{
  (void)writer;

  greenstone::dds::GUID endpoint_guid;
  endpoint_guid.from_octet16(status.last_subscription_handle().value);
  if(status.current_count_change() == 1) {
    publisher_event_->track_unique_subscription(endpoint_guid);
  } else if(status.current_count_change() == -1) {
    publisher_event_->untrack_unique_subscription(endpoint_guid);
  } else {
    return;
  }

  publisher_event_->update_matched(status.total_count(),
...
void CustomDataWriterListener::on_offered_deadline_missed(
  greenstone::dds::DataWriter *writer,
  const greenstone::dds::OfferedDeadlineMissedStatus & status) noexcept
{
  (void)writer;
...
void CustomDataWriterListener::on_liveliness_lost(
  greenstone::dds::DataWriter *writer,
  const greenstone::dds::LivelinessLostStatus & status) noexcept
{
  (void)writer;
...
void CustomDataWriterListener::on_offered_incompatible_qos(
  greenstone::dds::DataWriter *writer,
  const greenstone::dds::OfferedIncompatibleQosStatus & status) noexcept
{
  (void)writer;
...
greenstone::dds::StatusCondition * RMWPublisherEvent::get_statuscondition() const
{
...
bool RMWPublisherEvent::take_event(rmw_event_type_t event_type, void *event_info)
{
  ...
  switch(event_type) {
    ...
    case RMW_EVENT_PUBLICATION_MATCHED: {
        ...
        greenstone::dds::PublicationMatchedStatus matched_status;
...
void RMWPublisherEvent::untrack_unique_subscription(greenstone::dds::GUID guid)
{
  std::lock_guard<std::mutex> lock(subscriptions_mutex_);
...
void RMWPublisherEvent::update_liveliness_lost(uint32_t total_count, uint32_t total_count_change)
{
  ...
  // Accumulate deltas
  liveliness_lost_status_.total_count_change(liveliness_lost_status_.total_count_change() +
                                             total_count_change);
