...
__rmw_create_wait_set(const char * identifier, rmw_context_t * context, size_t max_conditions)
{
  ...
  greenstone::dds::WaitSet * swiftdds_wait_set = nullptr;
  ...
  wait_set->data = rmw_allocate(sizeof(greenstone::dds::WaitSet));
  if (!wait_set->data) {
  ...
  RMW_TRY_PLACEMENT_NEW(
    ...
    greenstone::dds::WaitSet, );
  (void)swiftdds_wait_set;
...
rmw_ret_t __rmw_destroy_wait_set(const char * identifier, rmw_wait_set_t * wait_set)
{
  ...
  if (wait_set->data) {
    if (swiftdds_wait_set) {
      RMW_TRY_DESTRUCTOR(
        swiftdds_wait_set->greenstone::dds::WaitSet::~WaitSet(),
        swiftdds_wait_set,
