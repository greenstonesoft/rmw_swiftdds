...
#include "rmw_swiftdds_cpp/init_rmw_context_impl.hpp"
...
static rmw_ret_t init_context_impl(rmw_context_t * context)
{
  ...
  // Improve graph discovery by using a unique listening port for its
  // subscription
  ...
  participant_info(
    ...
    [&](CustomParticipantInfo * participant_info) {
      if (RMW_RET_OK != rmw_swiftdds_shared_cpp::destroy_participant(participant_info)) {
  ...
  std::unique_ptr<rmw_publisher_t, std::function<void(rmw_publisher_t *)>> publisher(
    rmw_swiftdds_cpp::create_publisher(
      ...
      &publisher_options),
    [&](rmw_publisher_t * pub) {
      if (RMW_RET_OK != rmw_swiftdds_shared_cpp::destroy_publisher(
        greenstone_swiftdds_identifier, participant_info.get(), pub))
      {
        RCUTILS_SAFE_FWRITE_TO_STDERR(
  ...
  // If we would have support for keyed topics, this could be KEEP_LAST and
  // depth 1.
  qos.history = RMW_QOS_POLICY_HISTORY_KEEP_ALL;
  std::unique_ptr<rmw_subscription_t, std::function<void(rmw_subscription_t *)>> subscription(
    rmw_swiftdds_cpp::create_subscription(
      participant_info.get(),
      rosidl_typesupport_cpp::get_message_type_support_handle<ParticipantEntitiesInfo>(),
      "ros_discovery_info",
      &qos,
    ...
    [&](rmw_subscription_t * sub) {
      if (RMW_RET_OK != rmw_swiftdds_shared_cpp::destroy_subscription(
        greenstone_swiftdds_identifier, participant_info.get(), sub))
      {
  ...
  graph_guard_condition(
    rmw_swiftdds_shared_cpp::__rmw_create_guard_condition(greenstone_swiftdds_identifier),
    [&](rmw_guard_condition_t * p) {
      rmw_ret_t ret = rmw_swiftdds_shared_cpp::__rmw_destroy_guard_condition(p);
      if (ret != RMW_RET_OK) {
        RMW_SAFE_FWRITE_TO_STDERR(
  ...
  participant_guid.from_octet16(participant_info->participant_->get_instance_handle().value);
  common_context->gid =
    rmw_swiftdds_shared_cpp::create_rmw_gid(greenstone_swiftdds_identifier, participant_guid);
  ...
  rmw_ret_t ret = rmw_swiftdds_shared_cpp::run_listener_thread(context);
  if (RMW_RET_OK != ret) {
    return ret;
  }
  ...
  common_context->graph_cache.set_on_change_callback(
    [guard_condition = graph_guard_condition.get()]() {
      rmw_swiftdds_shared_cpp::__rmw_trigger_guard_condition(
        greenstone_swiftdds_identifier,
        guard_condition);
    });
...
rmw_ret_t rmw_swiftdds_cpp::increment_context_impl_ref_count(rmw_context_t * context)
{
  assert(context);
  assert(context->impl);
