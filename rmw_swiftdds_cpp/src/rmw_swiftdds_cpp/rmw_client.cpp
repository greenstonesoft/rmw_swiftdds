  ...
  const rmw_qos_profile_t * qos_policies)
{
  ...
  std::string response_topic_name =
    _create_topic_name(qos_policies, ros_service_response_prefix, service_name, "Reply");
  ...
  if (participant_info->participant_->is_type_registered(
      all_types.first->get_type_name(),
      all_types.first->type_identifier()))
  {
  ...
  // Debug info
  RCUTILS_LOG_DEBUG_NAMED("rmw_swiftdds_cpp", "************ Client Details *********");
  RCUTILS_LOG_DEBUG_NAMED("rmw_swiftdds_cpp", "Sub Topic %s", response_topic_name.c_str());
  RCUTILS_LOG_DEBUG_NAMED("rmw_swiftdds_cpp", "Pub Topic %s", request_topic_name.c_str());
  RCUTILS_LOG_DEBUG_NAMED("rmw_swiftdds_cpp", "***********");

  auto wih = info->request_writer_->get_instance_handle();
  info->writer_guid_.from_octet16(wih.value);
  auto rih = info->response_reader_->get_instance_handle();
  info->reader_guid_.from_octet16(rih.value);
  ...
  rmw_client->implementation_identifier = greenstone_swiftdds_identifier;
  ...
  // Update graph
  rmw_gid_t request_publisher_gid =
    rmw_swiftdds_shared_cpp::create_rmw_gid(greenstone_swiftdds_identifier, info->writer_guid_);
  rmw_gid_t response_subscriber_gid =
    rmw_swiftdds_shared_cpp::create_rmw_gid(greenstone_swiftdds_identifier, info->reader_guid_);

  {
    // Update graph
    std::lock_guard<std::mutex> guard(common_context->node_update_mutex);
    rmw_gid_t request_publisher_gid = rmw_swiftdds_shared_cpp::create_rmw_gid(
      greenstone_swiftdds_identifier, info->writer_guid_);
    ...
    rmw_gid_t response_subscriber_gid = rmw_swiftdds_shared_cpp::create_rmw_gid(
      greenstone_swiftdds_identifier, info->reader_guid_);
    rmw_dds_common::msg::ParticipantEntitiesInfo msg =
      ...
      node->name,
      node->namespace_);
    rmw_ret_t ret = rmw_swiftdds_shared_cpp::__rmw_publish(
...
rmw_ret_t rmw_destroy_client(rmw_node_t * node, rmw_client_t * client)
{
  ...
  return rmw_swiftdds_shared_cpp::__rmw_destroy_client(
    greenstone_swiftdds_identifier, node, client);
}
  ...
  rmw_qos_profile_t * qos)
{
  ...
  return rmw_swiftdds_shared_cpp::__rmw_client_request_publisher_get_actual_qos(client, qos);
}
  ...
  rmw_qos_profile_t * qos)
{
  ...
  return rmw_swiftdds_shared_cpp::__rmw_client_response_subscription_get_actual_qos(client, qos);
}
  ...
  const void * user_data)
{
  ...
  return rmw_swiftdds_shared_cpp::__rmw_client_set_on_new_response_callback(
