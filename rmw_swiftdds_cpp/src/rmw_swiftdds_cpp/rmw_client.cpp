  ...
  const rmw_qos_profile_t *qos_policies)
{
  ...
  if(RMW_RET_OK !=
    common_context->add_client_graph(
         request_publisher_gid, response_subscriber_gid, node->name, node->namespace_))
  {
    return nullptr;
  }
  ...
  if(TRACETOOLS_TRACEPOINT_ENABLED(rmw_client_init)) {
    rmw_gid_t gid{};
    rmw_swiftdds_shared_cpp::copy_from_dds_guid_to_byte_array(info->reader_guid_, gid.data);
