  ...
  bool keyed)
{
  ...
  if (!info->data_reader_listener_) {
    RMW_SET_ERROR_MSG(
      "create_subscription() could not create subscription data reader "
      "listener");
    return nullptr;
  }
  ...
  if (!rmw_swiftdds_shared_cpp::create_datareader(
