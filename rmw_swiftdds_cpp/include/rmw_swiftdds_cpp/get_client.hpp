...
#ifndef RMW_SWIFTDDS_CPP__GET_CLIENT_HPP_
#define RMW_SWIFTDDS_CPP__GET_CLIENT_HPP_
...
/// Return a native DDS DataWriter handle for the request.
/**
...
greenstone::dds::DataWriter * get_request_datawriter(rmw_client_t * client);

/// Return a native DDS DataReader handle for the response.
/**
 * The function returns `NULL` when either the client handle is `NULL` or
 * when the client handle is from a different rmw implementation.
 *
 ...
 */
RMW_SWIFTDDS_CPP_PUBLIC
greenstone::dds::DataReader * get_response_datareader(rmw_client_t * client);

}  // namespace rmw_swiftdds_cpp

#endif  // RMW_SWIFTDDS_CPP__GET_CLIENT_HPP_
